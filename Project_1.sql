...
select customerNumber,count(checkNumber) as total_no_of_payments,sum(amount) as total_paymentamount from payments
...
select customerNumber,sum(amount) as Total_payments from payments where year(paymentDate)='2004' group by
 customerNumber order by Total_payments desc limit 10;
...
select customerNumber,sum(amount) as Total_payments from payments where year(paymentDate)='2004' group by
 customerNumber order by Total_payments desc limit 10 offset 10;
...
select customerName,ucase(concat(contactlastName,contactfirstName)) as Full_Name,phone,country from customers where
...
select * from products;
select productCode,productName,buyPrice,round(((MSRP - buyPrice)*100/buyPrice),2) as Profit_Margin from products
...
select * from payments;
select customerNumber,max(amount) as largest_payments from payments where year(paymentDate)=2004 group by customerNumber 
order by largest_payments desc;
...
select date_format(paymentDate,"%M") as month_Name,year(paymentDate) as year,concat("$",format(sum(amount),1)) as total_payment
 from payments group by year,month_Name;
...
select c.customerNumber,customerName,e.employeeNumber as representativenumber,concat(e.firstName," ",e.lastName) as account_representative
...
select o.orderNumber,o.orderDate,p.productCode,p.productName from orders o
 ...
 select * from orders;
 select * from orderdetails;
 select o.orderNumber,customerName,sum(quantityOrdered*priceEach) as order_totalvalue from customers c join orders o on 
 c.customerNumber=o.customerNumber join orderdetails ord on o.orderNumber=ord.orderNumber group by orderNumber,customerName having order_totalvalue>25000;
